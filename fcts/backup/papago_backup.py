...
class dataProcessStream(object):
    ...
    def returnQuery(self, message) -> MutableSequence:
        ...
        return self.buildRequestInstane(
            basicQuery, needTranslate,
            self.translateDict[translateType]['ntl']['languageName'],
            self.translateDict[translateType]['tl']['languageName'])

    def buildRequestInstane(self, Query, needTranslate, ntl,
                            tl) -> MutableSequence:
        ...
        return self.checkResponseReturnDataBox(response, needTranslate, ntl,
                                               tl)

    def checkResponseReturnDataBox(self, response, needTranslate, ntl,
                                   tl) -> MutableSequence:
        dataBox = {
            "status": {
                "code": None

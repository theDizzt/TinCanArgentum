...
class InputCode(discord.ui.Modal, title='Level Killer Achievements Code'):
    code = discord.ui.TextInput(label='Input',
                                style=discord.TextStyle.short,
                                placeholder='Input your 9-digit code...')

    async def on_submit(self, interaction: discord.Interaction):
        codelist = lk_data['achievements']
        ...
        if result == None:
            ...
            tempid = result['id']
            temptitle = f"[{str(tempid).zfill(3)}] {result['title']}"
            xp = result['xp']
            money = result['money']
            if lk.readAchieve(interaction.user, tempid) == 1:
                await interaction.response.send_message(
                    f"`(⩌Δ ⩌ ;)` <@{interaction.user.id}> You have already unlocked this achievement!\nInfo: `{temptitle}`"
                )
            else:
                lk.achieveModify(interaction.user, tempid, 1)
...
class PaginationView(discord.ui.View):
    ...
    def create_embed(self, data, user):
        ...
        embed = discord.Embed(
            title=f"**{name}'s Level Killer Achievements**",
        ...
        for item in data:
            ...
            embed.add_field(
                name="[{}] {} {}".format(str(item['id']).zfill(3), item['title'],
                                    etc.checkBox(userdata[item['id']])),
                value=
                f"{desc}\n`Award` {prize}\n`Complete` {lk.readAchieveTime(user, item['id'])}",
                inline=False)
...
class LevelKiller(commands.Cog):  # Cog를 상속하는 클래스를 선언
    ...
    @commands.hybrid_command(name='lk', description="Level Killer related command")
    async def lk(self,
                    ctx,
                    option: str = "help",
                    value: int = 1,
        ...
        if option == "help" or option == "h":
            ...
            embed = discord.Embed(title="Level Killer Support Functions",
                                  description="Beta: 2020-12-19",
                                  color=0xFFFF72)

            embed.add_field(
                name="COMMANDS",
                value=
                "`;lk help` Show this page.\n`;lk achievements` Show and unlock your achievements list.\n`;lk hint` Get some hints.\n`;lk user` Show your profile.",
                inline=False)
            ...
            try:
                lk.newAchieve(ctx.author)
            except:
                None
            pagination_view = PaginationView(timeout=None)
            pagination_view.data = list(lk_data['achievements'].values())
            ...
            xp2 = etc.need_exp(lv) - etc.need_exp(lv - 1)
            text_xp = f"{xp1:,d} / {xp2:,d} ({100 * xp1 / xp2:.2f}%)"
            joindate = lk.readJoinTime(ctx.author)
            userdata = lk.achieveList(ctx.author)
            total = len(list(lk_data['achievements'].values()))
            unlocked = userdata[1:].count(1)

            embed = discord.Embed(title=f"**{name}'s Level Killer Profile**",
            ...
            if sub == 0:
                ...
                embed = discord.Embed(
                    title=f"Level Killer Level {value} Hints",
                    description=f"Your balance: ${q.readMoney(ctx.author):,d}",
                    color=0xFFFF72)

                embed.add_field(
                    name=
                    f"Type `;lk hint {value} <hint_number>` to get hints.",
                    value=
                ...
                if q.readMoney(ctx.author) >= value * 120:
                    ...
                    embed = discord.Embed(title=f"Level Killer Level {value} Hints",
    ...
    @commands.command(name='lkaccount', description="")
...
async def setup(client):
    await client.add_cog(LevelKiller(client))
